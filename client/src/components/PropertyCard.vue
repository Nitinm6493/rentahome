    ...
    <figure class="relative h-52">
      <router-link :key="property.id" :to="'/properties/' + property.id">
        <img
          ...
          class="card-image object-cover w-full h-full"
        />
      </router-link>
    </figure>
    <div class="card-body">
      ...
      <div class="flex flex-row justify-between">
        ...
        <span class="wishlist" @click="addItemToWishlist(property.id)">
          <i
            :class="{
              'bx bxs-heart bx-sm': is_favorite,
              'bx bx-heart bx-sm': !is_favorite,
            }"
          ></i>
        </span>
      </div>
...
export default {
  ...
  data() {
    return {
      WishList_API_BASE_URL: "http://localhost:8000/api/wishlist/",
      is_favorite: false,
    };
  },
  methods: {
    ...
    async addItemToWishlist(propertyId) {
      this.toggleWishlist();
      axios
        .post(this.WishList_API_BASE_URL, {
