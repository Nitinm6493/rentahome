  ...
  <div class="hero bg-base-200">
    <div class="hero-content flex flex-col justify-center">
      <div class="max-w-md text-center">
        ...
        <p class="py-6">
          Check out our latest properties for sale and rent today.
        </p>
      </div>
      <!-- Property Cards -->
...
export default {
  ...
  methods: {
    ...
    async getRecentProperties() {
      await axios
        .get("http://localhost:8000/manage-properties/recent-properties")
        .then((response) => {
          this.properties = response.data;
          //add new attribute to each property (title: category + city)
          for (let i = 0; i < this.properties.length; i++) {
            this.properties[i].title =
              this.properties[i].category +
              " in " +
              this.properties[i].location.city;
          }
          //get each property's image
