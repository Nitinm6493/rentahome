<template>
  <div>
    <h1 class="w-full text-center font-bold text-2xl my-5">Properties</h1>
    <!-- TODO: Add filter options in sidebar -->
    <div class="flex flex-col lg:flex-row">
      <div
        class="filter-bar w-full lg:w-1/4 px-4 rounded-lg border border-gray-400 py-3 ml-2 shadow-lg"
      >
        <!-- Search -->
        ...
        >
          ...
          <option v-for="category in property_categories" :key="category">
            {{ category }}
          </option>
        </select>
        <!-- Filter by City -->
      ...
      <div class="flex flex-col w-full content-center mx-4">
        <div
          class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2 flex justify-center mx-auto"
        ...
        <div
          class="alert alert-warning shadow-lg flex flex-col items-center justify-center"
          v-else
        >
          <div>
            <svg
...
export default {
  ...
  data() {
    return {
      ...
      PROPERTY_API_BASE_URL: `http://localhost:8000/manage-properties/properties`,
      property_categories: [
        "House",
        "Apartment",
        "Condo",
  ...
  methods: {
    async getProperties() {
      await axios
        .get(this.PROPERTY_API_BASE_URL)
        .then((response) => {
          this.properties = response.data;
          this.properties.forEach((property) => {
            property.publish_date = new Date(
    ...
    async getPropertyImage() {
      ...
      for (let i = 0; i < this.properties.length; i++) {
        await axios
          .get(
            "http://localhost:8000/uploads/properties/" +
    ...
    async filterByPriceRange() {
      await axios
        .post(
          "http://localhost:8000/manage-properties/filter-by-price",
          this.filter.price,
          {
            headers: {
    ...
    async filterByCategory() {
      ...
      axios
        .post(
          "http://localhost:8000/manage-properties/filter-by-category",
          category,
          {
            headers: {
    ...
    async filterByCity() {
      ...
      await axios
        .post("http://localhost:8000/manage-properties/filter-by-city", city, {
          headers: {
            "Content-Type": "text/plain",
          },
        })
    ...
    async filterByRating() {
      ...
      await axios
        .post(
          "http://localhost:8000/manage-properties/filter-by-rating",
          formDate
        )
        .then((response) => {
          this.properties = response.data;
    ...
    async filterBySearch() {
      ...
      await axios
        .post(
          "http://localhost:8000/manage-properties/filter-by-search",
          search,
          {
            headers: {
              "Content-Type": "text/plain",
    ...
    async getAllCities() {
      await axios
        .get("http://localhost:8000/manage-properties/cities")
        .then((response) => {
