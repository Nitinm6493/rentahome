        ...
        <form action="">
          <div class="card-body">
            <p class="text-secondary text-md">
              Please fill in the form below to reserve the property.
            </p>
            ...
            <div class="form-control mt-4">
              <button class="btn btn-primary" @click.prevent="reserve">
                Reserve
              </button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    async reserve() {
      const start_date_str = moment(this.start_date).format(
        "M/D/YYYY, h:mm:ss a"
      );
      ...
      await axios
        .post("http://localhost:8000/manage-reservation/add-reservation", data)
        .then((response) => {
    ...
    async getProperty() {
      await axios
        .get(
          "http://localhost:8000/manage-properties/properties/" +
