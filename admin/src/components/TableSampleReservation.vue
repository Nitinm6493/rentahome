...
<template>
  <CardBoxModal v-model="isModalActive" title="View Reservation Detail">
    <span> <b>Property ID :</b></span
    ><span> {{ selectedReservation.id }}</span> <br />
    <span> <b>Client Name :</b></span
    ><span> {{ selectedReservation.client.name }}</span> <br />
    <span> <b>Client Email :</b></span
    ><span> {{ selectedReservation.client.email }}</span> <br />
    <span> <b>Client Phone :</b></span
    ><span> {{ selectedReservation.client.phone }}</span> <br />
    <span> <b>Reservation will start from :</b></span
    ><span> {{ selectedReservation.start_date }}</span> <br />
    <span> <b>Reservation will end at :</b></span
    ><span> {{ selectedReservation.end_date }}</span> <br />
  </CardBoxModal>
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          <BaseButtons type="justify-start lg:justify-end" no-wrap>
            <BaseButton
              ...
              small
              @click="
                (isModalActive = true), (selectedReservation = reservation)
              "
            />
            <BaseButton
              color="success"
...
export default {
  ...
  data() {
    return {
      ...
      selectedReservation: {},
      pageSize: 5,
      RESERVATION_API_BASE_URL: "http://localhost:8000/manage-reservation",
    };
  },
  methods: {
    async getReservations() {
      await axios
        .get(this.RESERVATION_API_BASE_URL + "/reservations")
        .then((response) => {
    ...
    async approveReservation(id) {
      Swal({
        ...
        if (willDelete) {
          ...
          axios
            .put(this.RESERVATION_API_BASE_URL + "/update/" + id, reservation)
            .then((response) => {
    ...
    async deleteReservation(id) {
      await axios
        .delete(this.RESERVATION_API_BASE_URL + "/delete/" + id)
        .then((response) => {
          console.log(response.data);
          this.getReservations();
        })
