  ...
  <table>
    ...
    <tbody>
      <tr v-for="admin in paginatedAdmins" :key="admin.id">
        <TableCheckboxCell v-if="checkable" @checked="checked($event, admin)" />
...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 5,
      totalPages: 0,
      ADMIN_API_BASE_URL: "http://localhost:8000/manage-admin/admins",
    };
  },
  methods: {
    async getAdmins() {
      await axios
        .get(
          `http://localhost:8000/manage-admin/admins?page=${this.currentPage}&size=${this.pageSize}`
        )
