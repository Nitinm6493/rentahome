...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiPlus" title="New Property" main>
...
export default {
  ...
  data() {
    return {
      RESERVATION_API_BASE_URL: "http://localhost:8000/manage-properties",
      equipped: [
  ...
  methods: {
    async addNewProperty() {
      ...
      await axios
        .post(this.RESERVATION_API_BASE_URL + "/add-property", formData)
        .then(() => {
          swal({
    ...
    async getCategories() {
      await axios
        .get(this.RESERVATION_API_BASE_URL + "/categories")
        .then((response) => {
    ...
    async getRentTypes() {
      await axios
        .get(this.RESERVATION_API_BASE_URL + "/rent-types")
        .then((response) => {
    ...
    async latestPropertyId() {
      ...
      await axios
        .get(this.RESERVATION_API_BASE_URL + "/latest-property-id")
        .then((response) => {
