...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiPlus" title="Update Admin" main>
...
export default {
  ...
  methods: {
    getAdminById() {
      ...
      axios
        .get(`http://localhost:8000/manage-admin/admins/${id}`)
        .then((response) => {
    ...
    updateAdmin() {
      ...
      axios
        .put(
          `http://localhost:8000/manage-admin/update/${this.data.id}`,
          updatedAdmin
        )
        .then(() => {
          swal({
