...
const getTotalClients = async () => {
  const response = await axios.get(
    "http://localhost:8000/dashboard/clients-count"
  );
...
const getTotalProperties = async () => {
  const response = await axios.get(
    "http://localhost:8000/dashboard/properties-count"
  );
...
const getTodayReservations = async () => {
  const response = await axios.get(
    "http://localhost:8000/dashboard/today-reservations"
  );
...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      ...
      <SectionTitleLineWithButton
        :icon="mdiAccountMultiple"
        title="Newest subscibed clients"
      />
      <CardBox has-table>
        <TableNewestClients />
      </CardBox>
